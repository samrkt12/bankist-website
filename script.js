...
//smooth scrolling
btnScollTo.addEventListener('click', () => {
...
tabsContainer.addEventListener('click', function (e) {
  ...
  //Displaying active tab's content
  document
...
const handleHover = function (e) {
  if (e.target.classList.contains('nav__link')) {
    ...
    const siblings = hoverEl.closest('.nav').querySelectorAll('.nav__link');
    const logo = hoverEl.closest('.nav').querySelector('img');

    siblings.forEach(el => {
      if (el !== hoverEl) el.style.opacity = this;
    });
...
allLazyImages.forEach(img => imgObserver.observe(img));
...
//Functions
const slidesFunc = function (current) {
  slides.forEach((s, i) => {
...
const nextSlide = () => {
  if (curSlide === sliderLength - 1) curSlide = 0;
  else curSlide++;
  slidesFunc(curSlide);
...
const prevSlide = () => {
  if (curSlide === 0) curSlide = sliderLength - 1;
  else curSlide--;
  slidesFunc(curSlide);
  activateDot(curSlide);
};
